...
class MultiUAVEnvironment(object):
    ...
    def uav_move_and_energy_model(self, velocity, direction):
        ...
        for k in range(self.n_uav):
            ...
            # 根据公式计算功率消耗
            term1 = self.P0 * (1 + 3 * vk**2 / self.U_tip**2)
            term2 = self.P_ind * ((1 + vk**4 / (4 * self.v0**4))**0.5 - vk**2 / (2 * self.v0**2))**0.5
            term3 = 0.5 * self.d0 * self.rho * self.s_sol * self.A_area * vk**3
            self.power_consumption[k] = term1 + term2 + term3
    ...
    def transmit_data(self, alpha, theta, beta, phi) -> None:
            ...
            for i in self.uav_serv_gt[k]: # 遍历当前无人机下服务的所有用户
                ...
                for l in range(self.n_uav):
                    if l != k and self.cov_U2G[l][i] == 1: # 其他覆盖GT i的无人机会造成干扰
                        Io[i] += np.linalg.norm(self.H_U2G[l][i].conj().T @ P_comm[l]) ** 2 # 公有干扰
                        for j in self.uav_serv_gt[l]: # 私有干扰
                            Io[i] += np.linalg.norm(self.H_U2G[l][i].conj().T @ P_priv[l][j]) ** 2
                s = np.linalg.norm(self.H_U2G[k][i].conj().T @ P_comm[k]) ** 2
                self.shannon_capacity_c = shannon_capacity(s, Io[i] + Ip + self.n0)
        ...
        for k in range(self.n_uav):
            ...
            for i in self.uav_serv_gt[k]:
                ...
                self.shannon_capacity_p[i] = shannon_capacity(s, Io[i] + I_op + self.n0)

        # Eve窃听
        # 窃听公有信息
        self.shannon_capacity_c_e_k = np.zeros((self.n_eve, self.n_uav))
        I_e_o = np.zeros((self.n_eve))
        for e in range(self.n_eve):
            for k in range(self.n_uav):
                ...
                for l in range(self.n_uav):
                    if l != k and self.cov_U2E[l][e] == 1:
                        I_e_o[e] += np.linalg.norm(self.H_U2E[l][e].conj().T @ P_comm[l]) ** 2
                        for j in self.uav_serv_gt[l]:
                            I_e_o[e] += np.linalg.norm(self.H_U2E[l][e].conj().T @ P_priv[l][j]) ** 2
                self.shannon_capacity_c_e_k[e][k] = shannon_capacity(s, Ip + I_e_o[e] + self.n0)
        # 窃听私有信息
        self.shannon_capacity_p_e_i = np.zeros((self.n_eve, self.n_gt))
        for e in range(self.n_eve):
            for k in range(self.n_uav):
                ...
                for i in self.uav_serv_gt[k]:
                    ...
                    self.shannon_capacity_p_e_i[e][i] = shannon_capacity(s, Ip + I_e_o[e] + self.n0)
    
    def secrecy_rate_model(self):
        """安全模型"""
        self.sr_c = 0
        for k in range(self.n_uav):
            r_c_k = np.min(self.shannon_capacity_c[self.uav_serv_gt[k]])
            e = np.argmax(self.shannon_capacity_c_e_k[:, k])
            r_c_e = self.shannon_capacity_c_e_k[e, k]
            self.sr_c += self.cov_U2E[k][e] * max(0, r_c_k - r_c_e)
